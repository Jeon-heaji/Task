...
// JSONDecoder
if let dogs1 = try? JSONDecoder().decode([Dog].self, from: jsonArrData) {
    dogs1.forEach{ print($0) }
}
...
if let jsonObject = try? JSONSerialization.jsonObject(with: jsonDictData) as? [String: [[String: Any]]],
    let data1 = jsonObject["data"] as? [[String: Any]] {
        for object in data1 {
            if let name = object["name"] as? String,
                let age = object["age"] as? Int {
                let dog = Dog(name: name, age: age)
...
if let dog2 = try? JSONDecoder().decode([String:[Dog]].self, from: jsonDictData) {
    print(dog2)
}
