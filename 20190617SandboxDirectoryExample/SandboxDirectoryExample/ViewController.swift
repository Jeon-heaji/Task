...
class ViewController: UIViewController {
    ...
    @IBAction private func printBundleDirectory(_ sender: UIButton) {
        ...
        print("Bundle infoDectionary : \(info)\n")
    ...
    @IBAction private func printDocumentsDirectory(_ sender: UIButton) {
        ...
        let documentsUrl = fileManager.urls(
            ...
            in: .userDomainMask       // ios -> userDomainMask 쓰면 된당
            ).first!
        print(documentsUrl.path)            // bundle -> 개발자가 처음에 넣는것
    ...
    private func saveFileUrl() -> URL {
        ...
        if !fileManager.fileExists(atPath: saveDir.path) {  // 디렉토리가 존재하는지 확인
...
//        return URL(string: "")!
    ...
    @IBAction private func saveFile(_ sender: UIButton) {
        ...
        do {
            ...
            print(error.localizedDescription)       // 잘 되면 에러가 안뜸
        }
    }
    ...
    @IBAction private func loadFile(_ sender: UIButton) {
        ...
        let fileUrl = saveFileUrl()
        guard fileManager.fileExists(atPath: fileUrl.path) else { return print("File dosen't exist")}
...
extension ViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let melon = songsChart["Melon"],
            ...
            let splitArr = song.split(separator: "-", maxSplits: 1)  // - 제목에도들어가는경우
            cell.textLabel?.text = String(splitArr.last?.dropFirst() ?? "")
            // splitArr.last? -> 제목
            cell.detailTextLabel?.text = String(splitArr.first ?? "")
            
//            "A-B-C-D ".split(separator: "-")  // [A,B,C,D]
//            "A-B-C-D ".split(separator: "-", maxSplits: 1)  // [A,B-C-D]
        }
